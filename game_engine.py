...
class Minesweeper:
    ...
    def random_safe_reveal(self) -> None:
        safe_cells: List[Tuple[int, int]] = [
            (i, j) for i in range(self.n_rows) 
    ...
    def get_neighbors(self, i: int, j: int) -> List[Tuple[int, int]]:
        ...
        col_range = slice(max(0, j - 1), min(j + 2, self.n_cols))
        neighbors = [
            (x, y) for x in range(row_range.start, row_range.stop)
    ...
    def convert_minefield(self) -> np.ndarray:
        converted_minefield: np.ndarray = np.zeros((self.n_rows, self.n_cols), dtype=np.int8)
            ...
            for j in range(cols):
                ...
                if cell['state'] == State.COVERED.value:
                    ...
                    if cell['mine_count'] == -1:
                        ...
                        print(cell['mine_count'] if cell['mine_count'] >= 0 else ' ', end=' ')
