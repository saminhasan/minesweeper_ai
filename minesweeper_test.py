...
def P(s):
    ...
    return Permutation(
        (frozenset(sp[i]), int(sp[i + 1])) for i in range(0, len(sp[:-1]), 2)
    )
...
class Test(unittest.TestCase):
    ...
    def test_condense_supercells(self):
        def compare(raw_rules, out_rules, supercells):
            ...
            self.assertEqual(
                len(cells_), len(set(cells_))
            )  # list of cells should be unique
    ...
    def test_rule_reduce_metric(self):
        ...
        self.assertTrue(
            Reduceable(R("3:a,b,c,d,e"), R("1:a,b")).metric()
            > Reduceable(R("3:a,b,c,d"), R("2:b,c,d")).metric()
        )
        # then prefer bigger subrule
        self.assertTrue(
            Reduceable(R("3:a,b,c,d,e"), R("1:a,b,c")).metric()
            > Reduceable(R("3:a,b,c,d,e"), R("2:b,c")).metric()
        )
    ...
    def test_reduce_rules(self):
        ...
        self.assertEqual(
            reduce_rules([R("1:a,b"), R("1:a,c")]), set([R("1:a,b"), R("1:a,c")])
        )
        self.assertEqual(
    ...
    def test_permute(self):
        ...
        self.assertEqual(
            pset(R("1:a,b,c")), set([P("a1b0c0"), P("a0b1c0"), P("a0b0c1")])
        )
        self.assertEqual(
            pset(R("2:a,b,c")), set([P("a1b1c0"), P("a1b0c1"), P("a0b1c1")])
        )
        self.assertEqual(pset(R("3:a,b,c")), set([P("a1b1c1")]))
        self.assertEqual(pset(R("0:abc,de,f")), set([P("abc0de0f0")]))
        self.assertEqual(
            pset(R("1:abc,de,f")), set([P("abc1de0f0"), P("abc0de1f0"), P("abc0de0f1")])
        )
        self.assertEqual(
            pset(R("3:abc,de,f")),
            set(
        ...
        self.assertEqual(
            pset(R("5:abc,de,f")), set([P("abc3de2f0"), P("abc3de1f1"), P("abc2de2f1")])
        )
